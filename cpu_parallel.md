...
> **Note:** This document was originally written during the openmp-parallel branch development. The OpenMP features described here have been **fully integrated into the master branch** with additional enhancements:
> - Full-stack integration (CLI, Python bindings, Backend, Frontend)
> - Runtime threading control via `setUseParallel()` and `setNumThreads()`
> - Web UI dropdown for Single/Multi/GPU solver selection
> - 112 tests passing (up from 106)
> - See [INTEGRATION_SUMMARY.md](INTEGRATION_SUMMARY.md) for complete integration details
...
### 2. Run with CLI Threading Options
...
# Select "Single" or "Multi" from Solver dropdown
```
...
2. **Read the modified files** - See the pragmas in context:
   - `cpp/src/Ant.cpp` - Thread-local RNG (simplest change)
   - `cpp/src/PheromoneMatrix.cpp` - Parallel loops (clear examples)
   - `cpp/src/AntColony.cpp` - Full parallelization (most complex)
...
```
cpp/CMakeLists.txt              - OpenMP detection and linking
cpp/src/Ant.cpp                 - Thread-local RNG (1 change)
cpp/src/PheromoneMatrix.cpp     - Parallel evaporation + atomic deposition (2 changes)
cpp/src/AntColony.cpp           - Parallel ant construction + adaptive deposition (3 changes)
```
...
- **Tests:** All 112 passing
- **Full-stack:** CLI, Python, Web UI all support threading control
...
**Updated:** 2025-11-23 (integrated into master branch)
